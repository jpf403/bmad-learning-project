...
- Final verification (after all fixes/scope changes above): backend `dotnet test` — 168/168 passed (full suite, no regressions). Frontend `npx vitest run` — 136/136 passed (19 files, full suite). `npx eslint .` — clean. `npx prettier --check .` — clean.
