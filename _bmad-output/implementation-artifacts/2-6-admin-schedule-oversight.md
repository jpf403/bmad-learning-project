...
- [x] [Review][Patch] `MySchedule.jsx`'s `loading` branch unmounted the entire `date-header-row` — including the new Select Barber dropdown — during every barber switch and post-cancel refetch, replacing it with a plain "Loading…" paragraph, which dropped keyboard/screen-reader focus back to `<body>` mid-interaction. **Fixed**: restructured the render so the header row (nav arrows + admin dropdown) stays mounted once a `date` has ever been fetched successfully; only the content below the header (slot-list/loading/error) swaps on a reload. The true first-load state (before any `date` exists) still shows the original full-panel "Loading…"/error, unchanged. [frontend/src/pages/MySchedule.jsx:276-330]
- [x] [Review][Defer] No `aria-live` announcement when a barber switch completes — a screen-reader user gets silence, then the slot-list content changing, with nothing announcing "now viewing Barber X's schedule." Related to the focus-loss fix above but not fixed in this pass: the codebase has no existing `aria-live`/visually-hidden-status pattern to extend, so adding one is a small new pattern decision, not a drop-in fix. [frontend/src/pages/MySchedule.jsx] — deferred, no existing convention to reuse; worth revisiting alongside a broader accessibility pass.

- [x] [Review][Patch] Dead variable in `GetSchedule_barber_supplied_barberId_is_ignored` — `var barber = await FindByEmail("barber-login@example.com");` was assigned and never referenced again (copy-paste leftover). **Fixed**: removed the unused line. [backend/BarbershopApi.Tests/BookingControllerTests.cs:763]

- [x] [Review][Patch] `SelectDropdown`'s `avoidCollisions={false}` (pre-existing, unchanged) meant the admin barber-select's popover — anchored at the far right edge of the page — had no protection against rendering partially off-screen on narrow viewports. Root cause turned out bigger than the popover itself: `MySchedule.css` had zero `@media` breakpoints, unlike every other page in the app, despite `EXPERIENCE.md`'s `<640px` mobile-layout requirement — on a real phone width, the Admin header row (nav arrows + title + a 160px-min-width dropdown all in one `justify-content: space-between` line) doesn't fit. **Fixed**: added `@media (max-width: 639px)` stacking for `.my-schedule--admin .date-header-row` (nav-group above the barber dropdown, both full-width), matching the established stacking pattern already used in `NavBar.css:97-104`. No screenshot verification was done (no browser-automation tooling in this environment/repo; Jack opted to check visually himself rather than have Playwright installed for a one-off screenshot). [frontend/src/pages/MySchedule.css:54-73]
- [x] [Review][Defer] The zero-barbers state ("No barbers available.") has no retry affordance, unlike the barbers-fetch-failed state which offers "Try again" — a transient failure that resolves to "zero barbers" with no path to recheck without navigating away and back. [frontend/src/pages/MySchedule.jsx:266-273] — deferred, low-priority UX gap; zero-barbers is primarily a CI/test-suite condition per this story's own Dev Notes, not a state Jack's seeded local dev DB normally reaches.
...
- 2026-08-11: Code review round 1 — fixed 2 patch findings: (1) `MySchedule.jsx`'s loading state no longer unmounts the header row (nav arrows + admin Select Barber dropdown) on every barber switch/post-cancel refetch, which was dropping keyboard/screen-reader focus mid-interaction — the header now stays mounted once a date has ever loaded successfully, and only the content below it (slot-list/loading/error) swaps during a reload; (2) removed a dead `barber` variable in `GetSchedule_barber_supplied_barberId_is_ignored`. 2 items deferred (no `aria-live` barber-switch announcement — no existing pattern to extend; `SelectDropdown`'s `avoidCollisions={false}` at the admin trigger's new position — pre-existing component config). 1 item deferred as pre-existing/low-priority (no retry affordance for the zero-barbers state). Full regression suite green (176 backend, 146 frontend); ESLint/Prettier clean.
- 2026-08-11: Post-review mobile fix (requested by Jack, who flagged that this project must work end-to-end on a phone) — `MySchedule.css` had zero `@media` breakpoints, unlike every other page, so the Admin header row (nav arrows + title + a 160px-min-width barber-select dropdown, all in one `justify-content: space-between` line) didn't fit a real phone width, leaving the dropdown squeezed against the far-right edge with no room for its popover. Added `@media (max-width: 639px)` stacking for `.my-schedule--admin .date-header-row`, matching `NavBar.css`'s existing mobile-stacking pattern. CSS-only; frontend suite still green (23/23 for `MySchedule.test.jsx`), Prettier clean. Not screenshot-verified — no browser-automation tooling in this environment; Jack to spot-check visually.
- 2026-08-11: CI fix — `GetSchedule_without_date_param_defaults_to_today` (pre-existing since Story 2.5, not introduced by this story) asserted against `DateTime.Today`, which reads the host/CI-runner's local time zone rather than the server's fixed America/New_York clock (AD-12). On GitHub Actions' UTC runners this flaked during the ~4-5 hour daily window where UTC's calendar day has already rolled over but Eastern's hasn't yet, and hit both a docs-only push to `main` and this branch's push. Fixed by computing the expected date via the same `TimeZoneInfo.ConvertTimeFromUtc(..., "America/New_York")` conversion the server itself uses, matching the existing precedent in `BookingServiceTests.cs:187`. Jack pushed the fix directly; CI is green on this branch. `main`'s copy of this same flaky test will be resolved once this branch merges.
