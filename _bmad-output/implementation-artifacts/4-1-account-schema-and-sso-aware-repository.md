...
- [x] [Review][Patch] Soft-deleted accounts permanently reserve their SSO identity — the new `(SsoProvider, SsoSubjectId)` unique index filters only `WHERE SsoProvider IS NOT NULL`, unlike `Email`'s index which also excludes `DeletedAt IS NULL`. **Fixed:** index filter now `SsoProvider IS NOT NULL AND DeletedAt IS NULL` (migration `FreeSsoIdentityOnSoftDelete`), and `AccountRepository.SoftDelete` clears `SsoProvider`/`SsoSubjectId`. New tests: `CreateOrLinkSsoAccount_after_soft_delete_of_previous_owner_creates_a_fresh_account_with_the_same_identity`, `SoftDelete_clears_SsoProvider_and_SsoSubjectId`. [backend/BarbershopApi/Data/BarbershopDbContext.cs:21-23, backend/BarbershopApi/Repositories/AccountRepository.cs:85-93]
- [x] [Review][Patch] Neither migration's `Down()` restores `trg_Accounts_RowVersion`. **Fixed:** empirically confirmed (via `dotnet-ef migrations script`) that a `Sql()` trigger-recreate placed anywhere inside `AddSsoFieldsToAccount.Down()` still runs before its own deferred table rebuild and gets wiped, the same EF/SQLite behavior that forced the `Up()`-side two-migration split — so a symmetric fix isn't possible within the existing two migrations. Added a new migration, `RestoreRowVersionTriggerOnFullRollback`, dated between `AddAccountEntity` and `AddAppointmentEntity` (Up() is an idempotent no-op re-assert of the trigger; Down() recreates it) — its `Down()` runs after both SSO migrations' `Down()` on a full rollback, the only slot where the recreate survives. Verified via a full `dotnet-ef migrations script` down to `AddAccountEntity`: `CREATE TRIGGER` now runs as the final statement. Forward path re-verified unchanged (full script `InitialCreate`→`FreeSsoIdentityOnSoftDelete` produces the identical end schema). Neither of these SSO migrations had been applied to the local dev DB yet (`dotnet-ef migrations list` showed all three still `Pending`), so no retroactive-application risk. [backend/BarbershopApi/Migrations/20260728201143_RestoreRowVersionTriggerOnFullRollback.cs]
- [x] [Review][Patch] `CreateOrLinkSsoAccount` has no `try/catch` around `SaveChangesAsync()` for unique-constraint violations, unlike `AuthService.Register`'s `DbUpdateException`/`SqliteException{SqliteErrorCode:19}` → `DuplicateEmailException` pattern. **Fixed:** added the same catch, translating to a new `SsoIdentityConflictException`. New test: `CreateOrLinkSsoAccount_creating_new_account_with_identity_already_claimed_by_another_active_account_throws_SsoIdentityConflictException`. [backend/BarbershopApi/Repositories/AccountRepository.cs:100-136, backend/BarbershopApi/Services/SsoIdentityConflictException.cs]
- [x] [Review][Patch] No test proves the new `(SsoProvider, SsoSubjectId)` unique index is enforced at the DB level. **Fixed:** added `Unique_index_rejects_two_active_accounts_sharing_the_same_SsoProvider_and_SsoSubjectId`. [backend/BarbershopApi.Tests/AccountRepositoryTests.cs]
- [x] [Review][Patch] `CreateOrLinkSsoAccount_links_to_existing_account_by_email_without_altering_PasswordHash` never asserts the existing account's `FirstName`/`LastName` survive the link unchanged. **Fixed:** added `CreateOrLinkSsoAccount_linking_preserves_the_existing_account_Name`. [backend/BarbershopApi.Tests/AccountRepositoryTests.cs]
- [x] [Review][Patch] `FindBySsoIdentity_returns_null_when_no_match` only tests an empty database. **Fixed:** added `FindBySsoIdentity_does_not_match_on_partial_provider_or_subject_match`. [backend/BarbershopApi.Tests/AccountRepositoryTests.cs]
- [x] [Review][Defer] Re-linking an already-SSO-linked account silently rebinds it to a new identity with no conflict check [backend/BarbershopApi/Repositories/AccountRepository.cs:123-124] — deferred, pre-existing design gap not reachable by any current caller (single provider, stable per-user `subjectId`); relevant only if a second SSO provider is added later.
- [x] [Review][Defer] `context.Update(existing)` on link bumps `RowVersion` via the trigger, so a concurrent unrelated profile edit on the same account can get a false `409` — deferred, this is AD-16's optimistic-concurrency mechanism working as designed (first commit wins) for two writes racing the same row, not a defect; window is narrow (only an account's first-ever SSO link).
- [x] [Review][Defer] `provider`/`subjectId` aren't trimmed/normalized like `email` is [backend/BarbershopApi/Repositories/AccountRepository.cs:94-98,123-124] — deferred; `provider` is an internal constant we control, and OIDC `sub` claims must be compared as an exact opaque string per spec, so case-normalizing would be spec-incorrect. Revisit only if story 4.2's OAuth client shows real whitespace/casing drift.
- [x] [Review][Defer] Admin-invariant check in `CreateOrLinkSsoAccount` reads `existing.Role` directly instead of the `AsNoTracking` `EnsureNotCurrentlyAdmin` convention used by `AdminUpdate`/`SoftDelete` [backend/BarbershopApi/Repositories/AccountRepository.cs:118-121] — deferred; currently correct since `existing` is loaded fresh within the same call (no staleness window today), a style/consistency nit rather than a bug.
- [x] [Review][Defer] The trigger-preservation fix relies on an empirically observed, undocumented EF Core SQLite operation-batching order, fragile to a future EF Core upgrade — deferred; already mitigated by a regression test (`Update_increments_RowVersion`) and an explicit code comment for future maintainers, no better alternative exists in EF's public SQLite migration API today.
...
**Backend — new (from code review patches, 2026-08-21):**
- `backend/BarbershopApi/Services/SsoIdentityConflictException.cs`
- `backend/BarbershopApi/Migrations/20260821142743_FreeSsoIdentityOnSoftDelete.cs` (+ `.Designer.cs`) — updates the SSO unique index filter to also exclude soft-deleted rows
- `backend/BarbershopApi/Migrations/20260728201143_RestoreRowVersionTriggerOnFullRollback.cs` (+ `.Designer.cs`) — hand-authored, dated between `AddAccountEntity` and `AddAppointmentEntity`; its `Down()` is the only slot where recreating `trg_Accounts_RowVersion` survives a full rollback

**Backend — modified (from code review patches, 2026-08-21):**
- `backend/BarbershopApi/Repositories/AccountRepository.cs` — `SoftDelete` now clears `SsoProvider`/`SsoSubjectId`; `CreateOrLinkSsoAccount`'s create branch now catches the unique-constraint violation and throws `SsoIdentityConflictException`
- `backend/BarbershopApi/Data/BarbershopDbContext.cs` — SSO unique index filter now `SsoProvider IS NOT NULL AND DeletedAt IS NULL`
...
- 2026-08-21: Code review (3 layers: adversarial, edge-case, acceptance-audit) surfaced 2 decision-needed + 4 patch findings (4 defer, 3 dismissed as verified false positives/working-as-designed). Jack resolved both decisions in favor of fixing now: (1) soft-deleted accounts no longer permanently reserve their SSO identity — index filter and `SoftDelete` both updated (migration `FreeSsoIdentityOnSoftDelete`); (2) `trg_Accounts_RowVersion` now survives a full migration rollback via a new hand-authored migration, `RestoreRowVersionTriggerOnFullRollback`, inserted between `AddAccountEntity` and `AddAppointmentEntity` — verified empirically (via `dotnet-ef migrations script`) that no placement within the existing two SSO migrations' own `Down()` methods could survive their own deferred table rebuild, the same EF/SQLite behavior already known from Task 1's `Up()`-side fix. Also added: unique-constraint race handling in `CreateOrLinkSsoAccount` (new `SsoIdentityConflictException`, mirroring `AuthService.Register`'s pattern) and 3 test-coverage patches (DB-level unique index enforcement, name-preservation on link, `FindBySsoIdentity` partial-match). All fixes written test-first; full suite green at 272/272 (266 baseline + 6 new), clean rebuild 0 warnings. Story status → `done`.