...
- [x] **Task 1: Add "Sign in with z-pax" button + divider to the Login page** (AC: #1, #2, #4)
...
- [x] **Task 4: Check `deferred-work.md`**
  ...
  - [x] No other open item touches `Login.jsx`/the Auth frontend surface.
...
Amelia (Senior Software Engineer dev agent), Claude Sonnet 5.

### Debug Log References

None — no debugging required beyond the initial ESLint `set-state-in-effect` finding resolved during Task 2 (see Completion Notes).

### Completion Notes List

- Task 1: Added the "Sign in with z-pax" secondary `Button` and a CSS-only `.login__divider` (reusing existing border/muted-text/spacing tokens) after the existing form, inside `FormSection`. `handleSsoLogin` sets `window.location.href` directly (real browser navigation, not `navigate()`/`fetch`) per AC #2's full-redirect requirement.
- Task 2: Added `useSearchParams()` (first use of this hook in the codebase). Read the `error=sso_failed` query param via a lazy `useState` initializer for `formError` (mirroring the existing `successMessage` initializer pattern) rather than setting it inside a `useEffect` body — a plain effect-body `setFormError` call tripped ESLint's `react-hooks/set-state-in-effect` rule (cascading-render warning); the initializer approach reads the param exactly once on mount with no extra render and no lint suppression needed. A separate effect strips the `error` param from the URL via `setSearchParams({}, { replace: true })` so a refresh doesn't re-show the message.
- Task 3: All 4 new tests added to the existing `Login.test.jsx` (no new test file). Followed red-green-refactor per task: wrote failing tests for Task 1's button/divider/navigation behavior and Task 2's error-rendering behavior before implementing each, confirmed red, then implemented to green. Discovered `renderLogin`'s existing `initialEntries` array already accepts full location objects (`{ pathname, search }`) via `MemoryRouter` without any helper changes — the story's suggested "extend renderLogin" step turned out to be unnecessary; used the object form directly instead of adding a redundant parameter. Full frontend suite (192 tests, 20 files) green, `eslint .` clean, `prettier --check .` clean.
- Task 4: Re-read `deferred-work.md` in full. Confirmed the one `Login.jsx`-touching entry (Story 1.5 round 2's success-banner-on-remount gap, tied to the `successMessage` `useState` initializer) is unaffected — this story's `error`-param logic is a separate `useSearchParams` concern touching a different piece of state (`formError`'s initializer), not the `successMessage` remount path. Remains correctly deferred, not applicable here. No other open item touches the Auth frontend surface.
- Task 5: Branch `story/4.3-sign-in-with-zpax-login-page-ui` was already checked out at kickoff. Push/CI verification/PR/merge intentionally left for Jack per standing project practice (and the standing instruction to pause before commit/push/PR). No backend changes in this story — backend CI job is an unaffected pass-through.

### File List

- `frontend/src/pages/Login.jsx` (modified)
- `frontend/src/pages/Login.css` (modified)
- `frontend/src/pages/Login.test.jsx` (modified)
- `backend/BarbershopApi/Services/ZPaxSsoClient.cs` (modified, post-implementation — see Change Log)
- `backend/BarbershopApi/Controllers/AuthController.cs` (modified, post-implementation — see Change Log)
- `backend/BarbershopApi/appsettings.json` (modified, post-implementation — see Change Log)
- `backend/BarbershopApi.Tests/ZPaxSsoClientTests.cs` (modified, post-implementation — see Change Log)
- `backend/BarbershopApi.Tests/AuthControllerTests.cs` (modified, post-implementation — see Change Log)

## Change Log

- 2026-08-24: Implemented Story 4.3 (Tasks 1-5) — added the "Sign in with z-pax" secondary button and CSS-only divider to the Login page (real browser redirect via `window.location.href`, no client-side routing/fetch), and SSO-failure error rendering via a new `useSearchParams()` read of `?error=sso_failed` feeding the existing `formError`/`login__form-error` display through a lazy `useState` initializer (avoids an ESLint `set-state-in-effect` cascading-render warning that a plain effect-body `setFormError` call triggered). 4 new tests added to `Login.test.jsx`; full frontend suite green (192/192, 20 files), no regressions. No backend changes — frontend-only story, per its explicit scope (Story 4.2 already shipped the backend endpoints this story's UI consumes). Task 5 (push/PR/CI verification) intentionally left for Jack per standing project practice. This is the final story of Epic 4.
- 2026-08-26: **Post-implementation fix (live manual testing with real z-pax credentials).** This story's UI surfaced the first real end-to-end exercise of Story 4.2's backend SSO implementation, which had only ever run against `FakeSsoClient` in tests. Live testing found and fixed real-provider mismatches, all in `ZPaxSsoClient.cs`/`AuthController.cs` — outside this story's originally stated frontend-only scope (AD-19), but necessary to get the flow working at all:
  - `appsettings.json`: `AuthorizationEndpoint`/`TokenEndpoint`/`UserInfoEndpoint` hosts corrected from `sapi.auth.myzpax.com` to `dapi.auth.myzpax.com` per z-pax's confirmation that the provisioned client is registered against the `dapi` host.
  - `ZPaxSsoClient.cs`: the token endpoint's access-token field is real OAuth2 standard snake_case `access_token`, not the camelCase `accessToken` Story 4.2 had assumed (and hardcoded via case-insensitive matching, which does not bridge an underscore) — added `[JsonPropertyName("access_token")]`. This was silently returning an empty access token on every real login, which then 401'd on the userinfo call with no descriptive body; error-body logging was added to both the token and userinfo failure branches to make this class of issue visible in the future instead of a bare status code.
  - `ZPaxSsoClient.cs`: removed the `fallback_uri` param from the authorize request entirely — z-pax's real `/connect/authorize` endpoint rejects it (confirmed by reproducing the failure directly on z-pax's own site with and without this param).
  - `ZPaxSsoClient.cs`: `Scope` (used for the authorize request) changed from `api` to `profile` per z-pax's confirmed real scope value.
  - **Temporarily left in a debug state, explicitly at Jack's direction (not fixed further today):** the token-exchange request sends no `scope` field at all (`ZPaxSsoClient.cs`), and `state`/CSRF validation is bypassed end-to-end — omitted from the outgoing authorize request and the `AuthController.SsoCallback` cookie/mismatch/single-use check is skipped entirely. Both are marked `[DEBUG-TEMP]` at every call site. This is a real, deliberate CSRF-protection gap (login-CSRF: an attacker's own authorization code could be relayed through a victim's browser to log them into the attacker's linked account) accepted for now given this project's local-only, no-production-deploy scope (NFR7) — **must be restored before any deployment beyond local dev.** 4 `AuthControllerTests.cs` tests asserting state validation (`SsoCallback_missing_state_cookie_...`, `_mismatched_state_...`, `_state_already_consumed_...`, `_reuses_ssoState_cookie_only_once`) are `[Fact(Skip = "[DEBUG-TEMP] ...")]`'d rather than deleted, so they're ready to re-enable unchanged once state validation is restored. Corresponding assertions in `ZPaxSsoClientTests.cs` (the outgoing `state=`/token `scope=` params) were commented out with the same `[DEBUG-TEMP]` marker rather than asserting the now-temporarily-incorrect behavior as if it were correct.
  - Test fixtures in `ZPaxSsoClientTests.cs` updated from `accessToken` to `access_token` throughout to match the real fixed contract; `scope=api` assertions updated to `scope=profile` where still applicable.
  - Full backend suite re-verified: 288 passed, 4 skipped (all `[DEBUG-TEMP]`-justified), 0 failed, 292 total. Build clean (0 warnings on a full rebuild, aside from the expected `CS9113` unread-parameter warning on `ssoStateStore` while its only call site is bypassed).
