...
- 2026-08-10 (retroactive fix, found during Story 2.5's code review round 2): the barber/date-driven availability fetch (`useEffect` on `[barberId, date, user.accessToken]`) and the post-409-conflict availability re-fetch in `handleSubmit` had no cross-call staleness guard between them — unlike the mount effect's own `cancelled`-flag protection against *itself* re-firing, nothing stopped a slow 409-retry response from resolving after (and overwriting) a fresher response for a barber/date the user had since selected. Fixed by adding a `requestIdRef` generation counter: split the old single `loadAvailability` into a setState-free `fetchAvailability()` (shared) plus two setState-applying callers — the mount effect's own local wrapper, and a new `refreshAvailability()` for the 409-retry path — mirroring this same file's existing `fetchAppointments()`/`refreshAppointments()` split (see this file's own Review Findings below for why the setState calls can't just live in one shared function called from the effect). New test: `discards a stale 409-retry availability response after the barber selection has since changed`, confirmed to fail without the fix (the stale response reintroduced the old barber's slot) and pass with it. Found and fixed alongside the identical pattern in `MySchedule.jsx` (Story 2.5) — see that story's Review Findings for the full mechanism. Full regression suite green after the fix (171 backend, 140 frontend); ESLint/Prettier clean.
- 2026-08-10 (retroactive fix, found while implementing Story 2.5): the `isMountedRef` guard this story's review round added (see the first Review Finding above) had a latent React `StrictMode` bug — `useRef(true)` only sets the initial value once, and the guard's `useEffect` only ever *cleared* it to `false` on cleanup, never resetting it to `true` on StrictMode's dev-only synthetic mount→cleanup→mount cycle. Net effect: after that cycle, `isMountedRef.current` was permanently `false`, so `refreshAppointments()` silently no-opped after every cancel — the appointments list would not visually update to remove a cancelled row in dev, though `cancellingId` still reset correctly (it's not gated by the ref) so the Cancel button wasn't left disabled. Fixed by setting `isMountedRef.current = true` at the top of the mount effect. `ScheduleAppointment.test.jsx` now renders under `<StrictMode>` (matching `main.jsx`'s real tree) to guard against this bug class; reverting the fix and rerunning confirmed exactly 1 test failed (the post-cancel list-refresh test) before the fix. Found and fixed alongside the identical pattern in `MySchedule.jsx` (Story 2.5) — see that story's Dev Agent Record for the full mechanism. Full regression suite green after the fix (163 backend, 134 frontend).
