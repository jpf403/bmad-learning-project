...
| CSRF protection on the OAuth flow | Add a `state` parameter (random value, stored in a short-lived cookie, validated on callback) | Raised post-approval (2026-08-19): without it, an attacker could trick a victim into completing an SSO login the attacker initiated, potentially linking the victim's z-pax identity to an attacker-controlled account. Required by RFC 6749 §4.1.2 — a spec-compliant provider always echoes `state` back unmodified, so a missing/mismatched value on callback means it didn't originate from a flow this app initiated. Amendment applied to AD-19 and Story 4.2. |
...
Acceptance Criteria:
- Given the Login page, when a user clicks "Sign in with z-pax", then the backend generates a random `state` value, stores it in a short-lived cookie, and redirects the browser to z-pax's authorization endpoint (`https://sapi.auth.myzpax.com/connect/authorize`) with `client_id`, `scope=offline_access`, `response_type=code`, `redirect_uri`, and `state` (AD-19, FR42).
