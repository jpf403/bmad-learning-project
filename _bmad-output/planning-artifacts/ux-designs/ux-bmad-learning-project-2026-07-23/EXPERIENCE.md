...
| Admin account-create popup | Admin Panel, "Create Barber" action | Fields: email, first name, last name, password (double-entry, ×2 fields, required). No permission-level selector — creation always produces a barber account; there is no admin-creation or customer-creation path here (FR19). Save routes through the confirm-action popup (non-destructive Confirm). |
...
| Schedule row — booked (`{components.schedule-row-booked}`) | My Schedule | Shows the customer's name plus a destructive "Cancel" button. Cancel opens the confirm popup; confirming flips the row back to "Available" and frees the slot for booking (FR25–FR27). Once the slot's date/time has passed (computed "Finished," FR24), the row instead shows the customer's name plus a "Finished" label in place of the Cancel button — unlike My Appointments (above), My Schedule is a full-day grid and must keep showing who was booked in a past slot; it just can no longer be cancelled (server-enforced, not just UI). |
...
| Password mismatch | Register, Account, Admin edit-password, Admin create-account | "Passwords do not match," shown in `{typography.caption}`/`{colors.error}` (resolved 2026-07-27 — replaces the earlier plain-text default; `{colors.error}` shares its hex with `{colors.destructive}` but is a distinct token reserved for validation text only, never a button/fill). Only the two password fields require retyping. |
...
- No state relies on color alone: booked vs. open schedule rows are distinguished by text content (customer name + Cancel vs. "Available"), not just row tint; the password-mismatch and duplicate-email states pair `{colors.error}` with an explicit text message (see State Patterns) — color reinforces the message, it never carries it alone.
...
| `640–1023px` (tablet) | Content gutters narrow toward `{spacing.gutter-mobile}`. The 640px/1024px split itself is locked (client decision, 2026-07-27 — see DESIGN.md's Layout & Spacing); the nav-collapse *pattern* below this width remains this spine's own proposed default, not a separate client call: links remain inline as long as they fit, collapsing to a menu button only once they'd wrap, using the same custom-built styling as the rest of the nav. |
