...
| Booking form (barber / date / time / Submit) | Schedule Appointment | Barber select shows "No barbers available" in place of options if zero barber accounts exist (FR6). Calendar (`{components.calendar}`) disables past dates, dates beyond 30 days out, and weekends (FR7). Time dropdown (`{components.select-dropdown}`) lists only slots open for the chosen barber/date and, if the date is today, excludes anything within 30 minutes of the current time (FR8). Submit stays inactive until barber, date, and time are all chosen. |
...
| My Appointments list | Schedule Appointment, below the form | Signed-in user's own future bookings only, reusing `{components.schedule-row-booked}`'s visual treatment row-for-row (same border-only surface, same name-plus-Cancel layout) rather than a distinct visual component. Past bookings are retained, shown tagged "Finished," and never mixed into the same visual group as upcoming ones (FR24). Each upcoming row carries a `{components.button-destructive}` Cancel that opens the confirm popup. |
...
| Account page form | Account | Signed-in user edits own first name, last name, and password (double-entry) — not email (FR28). Save routes through the confirm-action popup (`{components.button-primary}` non-destructive Confirm, same as any other non-destructive save). Reuses `{components.input}` throughout; no dedicated visual component beyond that. |
...
| Finished appointments | Schedule Appointment (list) | Past bookings stay visible, tagged "Finished," visually separated from upcoming rows, no Cancel action available on them (FR24). |
...
| Barber demotion / account deletion cascade | Admin Panel edit (demote) or delete | Future (not-yet-occurred) appointments for that barber are cancelled and deleted; past/Finished appointments are retained as history in both cases (FR18/FR40). |
...
- No state relies on color alone: booked vs. open schedule rows are distinguished by text content (customer name + Cancel vs. "Available"), not just row tint; "Finished" appointments carry a text tag, not just muted color; the password-mismatch state is plain text by design (see State Patterns for why no dedicated error color exists).
