...
- **Hiding in-app Account/Logout UI for SSO-authenticated users** — deferred; currently the in-app Account and Logout controls remain visible and available to every account, SSO or password, as a fallback (FR48). Revisit during Story 4.6's review period whether SSO-authenticated visitors should have these hidden now that the myzPAX banner provides its own logout control. This decision reframes the banner from a nice-to-have nav convenience to the sole logout surface for SSO accounts — see the consolidated "Deferred to This Story's Review Period" section in `_bmad-output/implementation-artifacts/4-6-myzpax-banner-token-refresh.md` for the token-refresh-unification and degraded-banner-logout-availability questions this raises. **Hard precondition, noted by Jack (2026-09-08):** do not hide in-app Logout until either z-pax's own degraded fallback strip is confirmed to carry a working logout control, or this app adds its own logout affordance for the degraded-banner state (Story 4.6's `AuthContext.jsx` `else` branch — degraded banner, barbershop session left alive, no cookie present) — otherwise an SSO user whose banner degrades has no way to sign out at all.
