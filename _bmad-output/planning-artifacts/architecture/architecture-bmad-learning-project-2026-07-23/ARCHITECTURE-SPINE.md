...
- **Rule:** access token (JWT, 60-min expiry) held in memory only, sent as `Authorization: Bearer`; refresh token (JWT, 21-day expiry, carries SessionVersion) lives in an HttpOnly+Secure+SameSite=Strict cookie, never read by JS. `POST /api/auth/refresh` validates SessionVersion and mints a new access token, called on access-token expiry and on every fresh page load; `GET /api/auth/me` bootstraps identity for the frontend since the cookie is unreadable, returning exactly `{ id, email, firstName, lastName, role }` — no other endpoint invents its own shape for "who am I." Refresh tokens are non-rotating — a stolen refresh cookie stays valid up to 21 days or until the next password change (rotation/reuse-detection deferred, see Deferred). (Lifetime changed from 15 to 21 days, 2026-09-04, Story 4.6 — see AD-19 addendum for the matching z-pax-side change.)
...
- **Refresh-token rotation & reuse-detection** — accepted trade-off for now (AD-3); revisit as a future hardening pass if the stolen-cookie exposure window (up to 21 days) becomes unacceptable.
...
- ~~**Hiding in-app Account/Logout UI for SSO-authenticated users** — deferred...~~ **Resolved (2026-09-09, Story 4.6 code review, at Jack's direction).** Implemented as two independent rules rather than one: **Account editing** is durably blocked for any account with `SsoProvider` set (`AccountService.UpdateOwnProfile` throws `SsoAccountProtectedException` → `403`; `RequireRole`'s new `blockSso` prop redirects `/account` to `/` even on a direct URL type-in; `NavBar`'s "Account" item is `disabled`, not removed). **Logout** instead uses a *dynamic* condition tied to the banner's own live health (`isSsoLinked && zpaxAccessToken` present) — disabled only while the banner is actually up, re-enabling itself the instant the banner degrades. This dynamic design is what resolves the hard precondition noted by Jack (2026-09-08): it sidesteps ever needing to confirm z-pax's own degraded fallback strip has a working logout control, since this app's own in-app Logout becomes the reliable fallback the moment its banner is unhealthy — no separate "add our own affordance" work was needed. [frontend/src/components/NavBar.jsx, RequireRole.jsx; backend/BarbershopApi/Services/AccountService.cs, Controllers/AccountController.cs]
- ~~**myzPAX banner re-fetch on a stale token (FR47/AD-19)** — currently, once the one-time `zpaxAccessToken` goes stale/is consumed, the banner simply never mounts...~~ **Resolved** (2026-09-04, Story 4.6, Task 12/AC #8) — added mid-story after Jack found closing/reopening a tab left the banner permanently degraded for the rest of an SSO session. `AuthContext.jsx`'s bootstrap now falls back to the new `GET /api/auth/sso/zpax-refresh` (this story's own refresh mechanism, Tasks 3–5) whenever the one-time `zpax-token` pickup comes back empty, using the still-valid `zpaxRefreshToken` cookie to silently re-acquire a fresh token — exactly the future revision this item anticipated, now unblocked by this story's refresh work. [frontend/src/context/AuthContext.jsx]
